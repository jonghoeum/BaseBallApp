...
namespace BaseBallApp.API.Controllers
{
	...
	public class SupportersController : ControllerBase
	{
		...
		[HttpGet("Supporters")]
		public async Task<ActionResult<IEnumerable<SupportersViewModel>>> GetSupporterAsync([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10)
		{
			...
			try
			{
				var query = $@"
								...
								SELECT * 
								FROM SupportersWithRowNum
								WHERE NO BETWEEN {(pageNumber - 1) * pageSize + 1} AND {pageNumber * pageSize}
								ORDER BY NO;
							";

				var rawResult = await _db.SupportersViewModel.FromSqlRaw(query).ToListAsync();
		...
		[HttpPost("insert")]
		public async Task<IActionResult> InsertSupportersAsync([FromBody] SupportersClass request)
		{
			using var transaction = await _db.Database.BeginTransactionAsync();
			...
			{
				var sql = @"
                            INSERT INTO dbo.Supporters (isNotice, Title, Content, ViewCount, FileName1, FilePath1)
		...
		public async Task<IActionResult> IncrementViewCount([FromBody] int idx)
		{
			try
			{
				var Supporters = await _db.Supporters.FirstOrDefaultAsync(n => n.IDX == idx);
				if (Supporters == null)
					return NotFound();

				Supporters.ViewCount += 1;
		...
		[HttpPost("update")]
		public async Task<IActionResult> UpdateSupportersAsync([FromBody] SupportersClass request)
		{
			try
			{
				var Supporters = await _db.Supporters.FirstOrDefaultAsync(n => n.IDX == request.IDX);
				if (Supporters == null) return NotFound();

				Supporters.Title = request.Title;
				Supporters.Content = request.Content;
				Supporters.IsNotice = request.IsNotice;
				Supporters.FileName1 = request.FileName1;
				Supporters.FilePath1 = request.FilePath1;
				//Supporters.FileName2 = request.FileName2;
				//Supporters.FilePath2 = request.FilePath2;
		...
		// 공지 상세 조회
		[HttpGet("{id}")]
		public async Task<ActionResult<SupportersClass>> GetSupportersByIdAsync(int id)
		{
			try
			{
				var query = $@"
				...
				var result = await _db.Supporters.FromSqlRaw(query, param).FirstOrDefaultAsync();
		...
		public async Task<IActionResult> DeleteSupporters(int id)
		{
			try
			{
				var Supporters = await _db.Supporters.FirstOrDefaultAsync(n => n.IDX == id);
				if (Supporters == null) return NotFound();

				_db.Supporters.Remove(Supporters);
				await _db.SaveChangesAsync();
		...
		public async Task<IActionResult> Upload(IFormFile file)
		{
			...
			var uploadsFolder = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Supporters");
			...
			var relativePath = $"/Supporters/{uniqueFileName}"; // 클라이언트에 전달할 경로
